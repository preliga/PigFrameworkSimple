...
<div class="bg-faded" style="padding: 15px; text-align: center;">
    ...
    <form action="/admin/repertoire/bookTickets?showId={$show['id']}" method="post">
        {for $i = 0; $i < 5; $i++}
            <div class="text-center mt-4">
                {for $j = 0; $j < 8; $j++}
                    <div class="btn-group" data-toggle="buttons" style="margin: 10px;">
                        <label class="btn btn-success {if !empty($seats[$i * 8 + $j + 1])} active {/if}" style="width: 50px">
                            {$i * 8 + $j + 1}
                            <br>
                            <input type="checkbox" autocomplete="off" name="{$i * 8 + $j + 1}" {if !empty($seats[$i * 8 + $j + 1])} checked {/if} >
